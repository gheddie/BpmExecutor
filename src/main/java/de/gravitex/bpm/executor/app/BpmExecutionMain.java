	...
	public static void main(String[] args) {
		try {
			BpmExecutionSingleton.getInstance()
					.registerProcessDefinition(new BpmnDefinition(DEFAULT_SETTINGS, "SimpleTestProcess.bpmn", "SimpleTestProcess")
							.withCustomHandler("TASK#T1", new TaskT1Handler()).withBpmStateChecker("TASK#T1", new TaskT1BpmChecker()));
			BpmExecutionSingleton.getInstance()
					.registerProcessDefinition(new BpmnDefinition(DEFAULT_SETTINGS, "AnotherProcess.bpmn", "AnotherProcess")
							.withBpmStateChecker("TASK#TX", new TaskTxBpmChecker()));
			BpmExecutionSingleton.getInstance()
					.registerProcessDefinition(new BpmnDefinition(DEFAULT_SETTINGS, "LoopProcess.bpmn", "LoopProcess")
